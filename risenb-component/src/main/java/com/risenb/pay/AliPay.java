...
public class AliPay {
	 ...
	 */
	public static String appPay(String out_trade_no,String subject,String body,String total_fee) throws AlipayApiException{
		HashMap<String, String> map = new HashMap<String, String>();
		map.put("service", "mobile.securitypay.pay");//固定格式
		map.put("partner", PayConstant.ALI_PARTNER);//支付宝合作者身份ID
		map.put("_input_charset", "UTF-8");//字符编码  只有UTF-8
		
		map.put("notify_url", PayConstant.ALI_CALLBACK);//回调地址
		map.put("out_trade_no", out_trade_no);//商户订单号
		map.put("subject", subject);//商品名称
		map.put("payment_type", "1");//支付类型  商品购买
		map.put("seller_id", PayConstant.ALI_SELLER_ID);//卖家支付宝账号
		map.put("total_fee", total_fee);//支付总金额
		map.put("body", body);//商品详情
		
		String rsaSign = rsaSign(map, PayConstant.ALI_PRIVATE_KEY, "utf-8");
		map.put("sign_type", "RSA");//签名方式  目前只有RSA
		try {
	...
	/**
	 * 验证支付宝签名信息是否正确(异步回调使用)
	 * @param params
	 * @return
	 */
	public static boolean cheacekSign(Map<String, String> params){
		String string = params.get("sign");
		if(null ==string || "".equals(string) || "null".equals(string)){
			return false;
		}
		try {
			return rsaCheckV2(params, PayConstant.ALI_PUBLIC_KEY, "UTF-8");
	...
	/**
	 * 验证是否是支付宝发来的通知地址(异步回调使用)
	 * @param params
	 * @return
	 */
	public static boolean checkUrl(Map<String, String> params){
		String notifyId= params.get("notify_id");
		if(null == notifyId || "".equals(notifyId) || "null".equals(notifyId)){
			return false;
		}
		String veryfyUrl = PayConstant.ALI_HTTPS_VERIFY_URL + "partner=" + PayConstant.ALI_PARTNER + "&notify_id=" + notifyId;
		String resule = "";
	        ...
	        try {
	            ...
	            resule = in.readLine().toString();
	        } catch (Exception e) {
	            e.printStackTrace();
	            resule = "";
	        }
	   if("true".equals(resule)){
		   return true;
	   }
		return false;
	}
