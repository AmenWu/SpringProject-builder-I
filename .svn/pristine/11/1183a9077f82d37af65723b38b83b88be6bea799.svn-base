...
(function() {
	...
	var $WebUpload = function(pictureId) {
		...
		this.uploadUrl = $("#"+pictureId).attr("data-url");
	...
	$WebUpload.prototype = {
		...
		create : function() {
			var webUploader = WebUploader.create({
				...
				swf : g_path + 'js/plugins/webuploader/Uploader.swf',
		...
		bindEvent : function(bindedObj) {
			...
			bindedObj.on('uploadSuccess', function(file,response) {
				if(response.sesscess==true){//上传成功
					showSuccess("上传成功");
					$("#" + me.pictureId).val(response.url);
					$("#" + me.uploadPreId).empty();
					$("<i class='fa fa-times' data-id='"+me.pictureId+"' onclick='deleteImg(this);' style=' color:red; position:absolute; right:-1px; top:-3px; z-index:200; font-size: 22px; '></i><img   height='100px' width='100px' src='"+response.url+"'></img>").appendTo($("#" + me.uploadPreId));
				}else{
					showFailMsg(response.errorMsg);
					$("#" + me.uploadPreId).empty();
					var last = $("#" + me.pictureId).val();
					if(last != ""){
						$("<i class='fa fa-times' data-id='"+me.pictureId+"' onclick='deleteImg(this);' style=' color:red; position:absolute; right:-1px; top:-3px; z-index:200; font-size: 22px; '></i><img  height='100px' width='100px' src='"+last+"'></img>").appendTo($("#" + me.uploadPreId));
					}
				}
